...
// Define the Location Schema for GeoJSON format
...
const FreelancerSchema = new mongoose.Schema({
    uid: {
        type: String,
        unique: true,
        default: () => generateModelId(MODEL_PREFIXES.FREELANCER),
        index: true
    },
    userId: {
        type: String,
...
FreelancerSchema.index({ status: 1, 'profile.location.latitude': 1, 'profile.location.longitude': 1 });

// Index for verification status queries
FreelancerSchema.index({ verificationStatus: 1 });

// Index for combined queries
FreelancerSchema.index({ status: 1, verificationStatus: 1 });
FreelancerSchema.index({ countryId: 1, verificationStatus: 1 });
...
FreelancerSchema.methods.generatePasswordResetToken = function () {
    // Generate token
...
FreelancerSchema.methods.generateEmailVerificationToken = function () {
    // Generate token
...
FreelancerSchema.methods.hasPermission = function (permission) {
...
FreelancerSchema.methods.hasCountryAccess = function (countryId) {
